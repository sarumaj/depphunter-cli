...
describe('the introduction', () => {
  ...
  it('names a picture that the shot tool actually takes', () => {
    // The pictures are generated by scripts/tour-shots.mjs and named there; the cards
    // name them again here. Two lists, so they can come apart - and the way anybody
    // would find out is an introduction with a hole in it, on a first visit, which is
    // the one audience who cannot tell a missing picture from a map that looks like
    // that. Cheaper to read the tool.
    const tool = readFileSync(new URL('../../scripts/tour-shots.mjs', import.meta.url), 'utf8');
    const takes = new Set([...tool.matchAll(/shot\(page, '([\w-]+)'/g)].map(m => m[1]));
    assert.ok(takes.size >= 3, 'the shot tool takes almost no pictures');
    for (const deck of [TOUR, WALK_TOUR]) {
      for (const card of deck) {
        if (!card.shot) continue;
        assert.ok(takes.has(card.shot), `no picture called ${card.shot} is ever taken`);
        // A picture nobody can see needs words for what it shows.
        assert.ok(card.alt && card.alt.length > 20, `${card.shot} has no alt text worth reading`);
      }
    }
  });
  ...
  it('covers the keys somebody walking in has to know', () => {
    ...
    // at using either hand, or at getting back out. Three of these change hands, which
    // is three because they are for three different moments: the wheel to look at
    // them all, E and Q to reach for the next one without looking at any.
    for (const key of ['W, A, S and D', 'Shift', 'Space', 'Esc', '1 to 0',
      'R ', 'E ', 'Q ', 'H ', 'click', 'F, C', 'Enter', 'B ', '?']) {
