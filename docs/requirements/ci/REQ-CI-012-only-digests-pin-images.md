...
the tag as the requested version; a reference without a tag **shall** resolve to
version `latest`.

## Rationale

An image tag is republished whenever its owner likes, so `nginx:1.25.3` floats;
only an OCI digest is immutable. No tag at all means `:latest`, the loosest
reference there is.
...
3. `localhost:5000/app` resolves to package `localhost:5000/app`, version
   `latest`.
4. `nginx` resolves to version `latest`, floating.
