...
number of dependencies and who answered it: a lock file, an index, the on-disk
cache, an answer already given in this run, or nobody.
...
1. A lock-file answer, an index answer, a cached answer and a repeated question
   are recorded as `lock`, `index`, `cache` and `memo`.
